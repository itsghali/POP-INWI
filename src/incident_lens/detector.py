...
class IncidentDetector:
    ...
    def __init__(self, data: pd.DataFrame, temp_min: Optional[float] = None, temp_max: Optional[float] = None):
        """
        Initialize incident detector with configurable temperature thresholds.
        
        Args:
            data: DataFrame with monitoring data
            temp_min: Minimum acceptable temperature (°C). If None, uses default 20.0
            temp_max: Maximum acceptable temperature (°C). If None, uses default 26.0
        """
        self.data = data.copy()
        self.data = self.data.sort_index()  # Ensure chronological order
        self.incidents: List[Incident] = []
        self.custom_temp_min = temp_min
        self.custom_temp_max = temp_max
        self.thresholds = self._initialize_thresholds()
        
    def _initialize_thresholds(self) -> Dict[str, Dict[str, float]]:
        """Initialize detection thresholds with dynamic adjustments"""
        # Use custom temperature thresholds if provided, otherwise use defaults
        temp_max = self.custom_temp_max if self.custom_temp_max is not None else 26.0
        temp_min = self.custom_temp_min if self.custom_temp_min is not None else 20.0
        
        # Base thresholds
        base_thresholds = {
            'T°C AMBIANTE': {
                'max_threshold': temp_max,  # User-configured or default
                'min_threshold': temp_min,  # User-configured or default
                'optimal_range': (temp_min, temp_max),
                # Legacy fields kept for backward compatibility
                'critical_high': temp_max,
                'warning_high': temp_max,
                'warning_low': temp_min,
                'critical_low': temp_min
            },
            'T°C EXTERIEURE': {
                'extreme_high': 40.0,
                'high': 35.0,
    ...
    def set_temperature_thresholds(self, temp_min: float, temp_max: float):
        """Update temperature thresholds after initialization.
        ...
        self.custom_temp_min = temp_min
        self.custom_temp_max = temp_max
        self.thresholds['T°C AMBIANTE']['max_threshold'] = temp_max
        self.thresholds['T°C AMBIANTE']['min_threshold'] = temp_min
        self.thresholds['T°C AMBIANTE']['optimal_range'] = (temp_min, temp_max)
        # Update legacy fields for backward compatibility
        self.thresholds['T°C AMBIANTE']['critical_high'] = temp_max
        self.thresholds['T°C AMBIANTE']['warning_high'] = temp_max
        self.thresholds['T°C AMBIANTE']['critical_low'] = temp_min
        self.thresholds['T°C AMBIANTE']['warning_low'] = temp_min
    ...
    def _detect_temperature_incidents(self, data: pd.DataFrame) -> List[Incident]:
        """Detect temperature-related incidents with deviation-based severity"""
        incidents = []
        ...
        temp_max = thresholds['max_threshold']
        temp_min = thresholds['min_threshold']
        ...
        for idx, row in data.iterrows():
            ...
            if temp > temp_max:
                # TEMPERATURE HIGH - Severity based on deviation from threshold
                deviation = abs(temp - temp_max)
                
                if deviation >= 5.0:
                    severity = IncidentSeverity.EMERGENCY
                    severity_label = "URGENCE"
                elif deviation >= 2.0:
                ...
                incidents.append(Incident(
                    ...
                    affected_systems=['COOLING', 'IT_EQUIPMENT'] if severity in [IncidentSeverity.CRITICAL, IncidentSeverity.EMERGENCY] else ['COOLING'],
                    description=f"Température ambiante {severity_label}: {temp:.1f}°C (seuil: {temp_max}°C, déviation: {deviation:.1f}°C)",
                    context={
                        'deviation': deviation,
                ...
                # TEMPERATURE LOW - Severity based on deviation from threshold
                deviation = abs(temp - temp_min)
                ...
                incidents.append(Incident(
                    ...
                    description=f"Température ambiante {severity_label}: {temp:.1f}°C (seuil: {temp_min}°C, déviation: {deviation:.1f}°C)",
                    context={
                        'deviation': deviation,
                        'external_temp': row.get('T°C EXTERIEURE', None)
                    }
    ...
    def _detect_clim_incidents(self, data: pd.DataFrame) -> List[Incident]:
        ...
        for idx, row in data.iterrows():
            ...
            if active_clims == 0:
                # All CLIMs have failed - identify them all
                failed_clim_names = [col.replace('_Status', '').replace('CLIM_', '') for col in available_clim]
                ...
                incidents.append(Incident(
                    ...
                    affected_systems=['COOLING'] + [col.replace('_Status', '') for col in available_clim],
                    description=f"Panne totale du système CLIM - Aucune unité active - Pannes: {', '.join(failed_clim_names)}",
                    context={
                        'failed_units': available_clim,
                        'temperature': row.get('T°C AMBIANTE', None),
                        'total_units': total_clims
                    }
                ))
            
            elif active_clims < total_clims / 2:
                # Identify which specific CLIMs have failed
                failed_clims = [col for col in available_clim if row[col] == 0 or pd.isna(row[col])]
                failed_clim_names = [col.replace('_Status', '') for col in failed_clims]
                ...
                incidents.append(Incident(
                    ...
                    affected_systems=['COOLING'] + failed_clim_names,
                    description=f"Système CLIM dégradé - Seulement {active_clims}/{total_clims} unités actives - Pannes: {', '.join([n.replace('CLIM_', '') for n in failed_clim_names])}",
                    context={
                        'failed_units': failed_clims,
                        'active_units': active_clims,
                        'total_units': total_clims,
                        'temperature': row.get('T°C AMBIANTE', None)
                    }
                ))
        ...
        for idx, row in data.iterrows():
            ...
            if it_power >= thresholds['critical_high']:
                incidents.append(Incident(
                    ...
                    threshold_violated=thresholds['critical_high'],
                    duration_seconds=900,  # 15-min interval default
    ...
    def _detect_composite_incidents(self, data: pd.DataFrame) -> List[Incident]:
        ...
        if 'T°C AMBIANTE' in data.columns and any(col in data.columns for col in 
            ...
            for idx, row in data.iterrows():
                ...
                temp_max = self.thresholds['T°C AMBIANTE']['max_threshold']
                if temp > temp_max and len(failed_clims) >= 2:
                    incidents.append(Incident(
                        id=str(uuid.uuid4()),
                        timestamp=idx,
                        type=IncidentType.TEMPERATURE_HIGH,
