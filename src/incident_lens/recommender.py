...
class RecommendationEngine:
    ...
    def generate_recommendations_for_incident(self, incident) -> List[Recommendation]:
        """
        Generate recommendations directly from incident type when root causes are not available
        
        Args:
            incident: The incident object
            
        Returns:
            List of recommendations based on incident type
        """
        recommendations = []
        incident_type = incident.type.value if hasattr(incident.type, 'value') else str(incident.type)
        severity = incident.severity.value if hasattr(incident.severity, 'value') else str(incident.severity)
        
        # Map incident types to recommendations
        if incident_type == 'it_power_low':
            # IT Power Low recommendations
            priority = RecommendationPriority.URGENT if severity in ['emergency', 'critical'] else RecommendationPriority.HIGH
            
            recommendations.append(Recommendation(
                title='⚡ Vérification urgente puissance IT',
                description='Puissance IT anormalement basse détectée - risque de sous-charge ou défaillance mesure',
                priority=priority,
                type=RecommendationType.EMERGENCY_ACTION,
                estimated_time='30 minutes',
                responsible_team='Équipe technique sur site',
                steps=[
                    'Vérifier le fonctionnement des équipements IT',
                    'Contrôler les capteurs de puissance',
                    'Vérifier les PDU et tableaux électriques',
                    'S\'assurer qu\'aucun équipement critique n\'est éteint',
                    'Comparer avec la charge IT attendue'
                ],
                resources_needed=['Multimètre', 'Schéma électrique', 'Liste équipements IT'],
                expected_outcome='Identification de la cause de la faible consommation IT',
                related_causes=[incident_type]
            ))
            
            if severity in ['emergency', 'critical']:
                recommendations.append(Recommendation(
                    title='🔍 Audit équipements IT',
                    description='Puissance IT critique nécessite audit complet des équipements',
                    priority=RecommendationPriority.HIGH,
                    type=RecommendationType.PREVENTIVE_MAINTENANCE,
                    estimated_time='2 heures',
                    responsible_team='Administrateur système + Électricien',
                    steps=[
                        'Inventorier tous les équipements IT actifs',
                        'Vérifier l\'état de chaque serveur/équipement',
                        'Contrôler la précision des capteurs de mesure',
                        'Documenter les écarts entre charge attendue et mesurée',
                        'Planifier recalibrage ou remplacement capteurs si nécessaire'
                    ],
                    expected_outcome='Cartographie complète de la charge IT et identification des anomalies',
                    related_causes=[incident_type]
                ))
                
        elif incident_type == 'it_power_high':
            # IT Power High recommendations
            priority = RecommendationPriority.URGENT if severity in ['emergency', 'critical'] else RecommendationPriority.HIGH
            
            recommendations.append(Recommendation(
                title='💻 Analyse charge IT excessive',
                description='Puissance IT anormalement élevée - identifier et équilibrer la charge',
                priority=priority,
                type=RecommendationType.EMERGENCY_ACTION,
                estimated_time='1 heure',
                responsible_team='Administrateur système',
                steps=[
                    'Identifier les équipements à forte consommation',
                    'Vérifier les processus et tâches en cours',
                    'Équilibrer la distribution de charge si possible',
                    'Vérifier qu\'aucun équipement ne fonctionne anormalement',
                    'Planifier tâches intensives aux heures plus fraîches'
                ],
                expected_outcome='Réduction et optimisation de la charge IT',
                related_causes=[incident_type]
            ))
            
        elif incident_type in ['temperature_high', 'temperature_low']:
            # Temperature incident recommendations
            priority = RecommendationPriority.IMMEDIATE if severity in ['emergency', 'critical'] else RecommendationPriority.URGENT
            
            if incident_type == 'temperature_high':
                recommendations.append(Recommendation(
                    title='❄️ Intervention refroidissement',
                    description='Température ambiante trop élevée - action immédiate requise',
                    priority=priority,
                    type=RecommendationType.EMERGENCY_ACTION,
                    estimated_time='30 minutes',
                    responsible_team='Équipe technique',
                    steps=[
                        'Vérifier le fonctionnement des CLIMs',
                        'S\'assurer que les portes sont fermées',
                        'Contrôler la température extérieure',
                        'Vérifier la charge IT',
                        'Activer CLIMs de secours si disponibles'
                    ],
                    expected_outcome='Retour de la température dans la plage acceptable',
                    related_causes=[incident_type]
                ))
            else:
                recommendations.append(Recommendation(
                    title='🌡️ Température basse anormale',
                    description='Température ambiante trop basse - vérification nécessaire',
                    priority=priority,
                    type=RecommendationType.EMERGENCY_ACTION,
                    estimated_time='30 minutes',
                    responsible_team='Équipe technique',
                    steps=[
                        'Vérifier les réglages des CLIMs',
                        'S\'assurer que les CLIMs ne sur-refroidissent pas',
                        'Contrôler les capteurs de température',
                        'Vérifier la charge IT (sous-charge possible)',
                        'Ajuster les consignes CLIM si nécessaire'
                    ],
                    expected_outcome='Retour de la température dans la plage acceptable',
                    related_causes=[incident_type]
                ))
                
        elif incident_type in ['clim_failure', 'clim_degraded']:
            # CLIM incident recommendations
            priority = RecommendationPriority.IMMEDIATE if incident_type == 'clim_failure' else RecommendationPriority.URGENT
            
            recommendations.append(Recommendation(
                title='🚨 Intervention CLIM urgente',
                description='Système CLIM en défaillance - action immédiate requise',
                priority=priority,
                type=RecommendationType.EMERGENCY_ACTION,
                estimated_time='1 heure',
                responsible_team='Service maintenance CLIM',
                steps=[
                    'Identifier les unités en panne',
                    'Vérifier alimentation électrique',
                    'Contrôler disjoncteurs et fusibles',
                    'Tenter redémarrage des unités',
                    'Contacter mainteneur CLIM si nécessaire',
                    'Activer plan de contingence thermique'
                ],
                resources_needed=['Outils maintenance CLIM', 'Contact mainteneur'],
                expected_outcome='Restauration du refroidissement',
                related_causes=[incident_type]
            ))
            
        elif incident_type in ['door_anomaly', 'door_extended_open']:
            # Door incident recommendations
            priority = RecommendationPriority.URGENT if severity in ['emergency', 'critical'] else RecommendationPriority.HIGH
            
            recommendations.append(Recommendation(
                title='🚪 Intervention porte salle',
                description='Porte restée ouverte trop longtemps - action requise',
                priority=priority,
                type=RecommendationType.EMERGENCY_ACTION,
                estimated_time='15 minutes',
                responsible_team='Équipe sécurité / technique',
                steps=[
                    'Vérifier que la porte est bien fermée',
                    'Identifier la cause de l\'ouverture prolongée',
                    'Contrôler le système de fermeture automatique',
                    'Sensibiliser le personnel aux bonnes pratiques',
                    'Envisager alarme si ouvertures fréquentes'
                ],
                expected_outcome='Porte fermée et prévention récurrence',
                related_causes=[incident_type]
            ))
            
        elif incident_type in ['power_anomaly', 'pue_high', 'pue_critical']:
            # Power/PUE recommendations
            priority = RecommendationPriority.URGENT if severity in ['emergency', 'critical'] else RecommendationPriority.HIGH
            
            recommendations.append(Recommendation(
                title='⚡ Analyse consommation énergétique',
                description='Anomalie puissance/PUE détectée - audit nécessaire',
                priority=priority,
                type=RecommendationType.CONFIGURATION_CHANGE,
                estimated_time='2 heures',
                responsible_team='Responsable énergie + Technicien',
                steps=[
                    'Analyser la répartition de consommation',
                    'Vérifier efficacité refroidissement',
                    'Contrôler précision des mesures',
                    'Identifier sources de surconsommation',
                    'Proposer actions d\'optimisation énergétique'
                ],
                expected_outcome='Identification causes et plan d\'optimisation',
                related_causes=[incident_type]
            ))
            
        else:
            # Generic recommendation for unknown incident types
            recommendations.append(Recommendation(
                title='🔍 Investigation requise',
                description=f'Incident de type {incident_type} - analyse approfondie nécessaire',
                priority=RecommendationPriority.HIGH,
                type=RecommendationType.EMERGENCY_ACTION,
                estimated_time='1 heure',
                responsible_team='Expert technique',
                steps=[
                    ...
                    'Définir plan d\'action approprié'
                ],
                expected_outcome='Compréhension de l\'incident et plan d\'action',
                related_causes=[incident_type]
            ))
