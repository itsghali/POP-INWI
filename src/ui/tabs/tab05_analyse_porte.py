        ...
        if not door_events.empty:
                ...
                if len(porte_data_clean) > 0:
                    ...
                    if len(cycles_df) > 0:
                        ...
                        if 'open_ts' in cycles_display.columns:
                            cycles_display['open_ts'] = cycles_display['open_ts'].dt.strftime('%Y-%m-%d %H:%M:%S')
                        if 'close_ts' in cycles_display.columns:
                            cycles_display['close_ts'] = cycles_display['close_ts'].dt.strftime('%Y-%m-%d %H:%M:%S')
            ...
            with st.expander("🔍 Debug - Cycles détectés"):
                ...
                if len(cycles_df) > 0:
                    ...
                    display_df['open_time'] = display_df['open_ts'].dt.strftime('%Y-%m-%d %H:%M')
                    display_df['close_time'] = display_df['close_ts'].dt.strftime('%Y-%m-%d %H:%M')
                    
                    # Afficher tous les cycles si moins de 50, sinon les 50 premiers
                    n_display = min(len(display_df), 50)  # More concise way to get the minimum
            ...
            if len(porte_data_clean) > 0:
                ...
                st.plotly_chart(fig_status, use_container_width=True, key="door_status_timeline")
            ...
            if len(cycles_df) > 0:
                ...
                st.plotly_chart(fig_timeline, use_container_width=True, key="door_cycles_timeline")
                ...
                if door_impacts and len(door_impacts) > 0:
                    ...
                    if cycle_idx is not None:
                        ...
                        if len(cycle_data) > 0:
                            ...
                            st.plotly_chart(fig_selected, use_container_width=True, key=f"door_cycle_detail_{cycle_idx}")
                    ...
                    st.plotly_chart(fig, use_container_width=True, key="door_temp_evolution")
                    ...
                    with st.container():
                        ...
                        st.plotly_chart(fig, use_container_width=True, key="door_duration_correlation")
