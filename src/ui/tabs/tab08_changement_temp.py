                    ...
                    if 'Temp_Exterieure' in df.columns or 'Puissance_IT' in df.columns:
                        ...
                        if 'Temp_Exterieure' in df.columns:
                            ...
                            if len(corr_data_ext) >= 5:
                                # Utilise fonction robuste qui vérifie corrélation + variation ext + accord directionnel
                                ext_flag = exterior_causes_ambient_spike(
                                    corr_data_ext['Temp_Ambiante'].values,
                                    corr_data_ext['Temp_Exterieure'].values,
                                    spearman_thresh=0.5,
                                    min_ext_change=0.8
                                )
                                if ext_flag:
            ...
            if not low_candidates.empty:
                ...
                for _, cluster in low_candidates.groupby('cluster'):
                    ...
                    if 'Temp_Exterieure' in df.columns or 'Puissance_IT' in df.columns:
                        ...
                        # --- Calcul Ext ---
                        if 'Temp_Exterieure' in df.columns:
                            context_Ext = df[(df['Timestamp'] >= peak_row['Timestamp'] - pd.Timedelta(hours=1)) & 
                                               (df['Timestamp'] <= peak_row['Timestamp'] + pd.Timedelta(minutes=20))]
