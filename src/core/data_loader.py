...
@st.cache_data
def load_data(region, pop, silent=False):
    """Charge les données avec gestion robuste - VERSION COMPLÈTE
    
    Args:
        region: Région du POP
        pop: Nom du POP
        silent: Si True, supprime les messages UI (pour préchargement en masse)
    """
    try:
        cleaner = DataCleaner()
        ...
        # Chargement des données (sans UI si silent=True)
        ...
        # Afficher l'erreur si pas en mode silent
        if not silent:
            print(f"❌ Erreur chargement {region}/{pop}: {str(e)}")
        return {}, pd.DataFrame()
